...
*/
var YMPParams = {
	defaultalbumart:'http://nvzion.com/temp/default-cover.jpg'
};
...
function edpickQueryResults(results) {
    _digStyleQueryResults(results,'#edpicks', 'More picks&hellip;', 'picks.html', 'Ed picks');
}
function edpickPageQueryResults(results) {
	
}
function popchartQueryResults(results) {
	_digStyleQueryResults(results,'#popchart', 'More popular&hellip;', 'popular.html', 'Popular');
}
function podcastQueryResults(results) {
	...
	html += '<p><a href="podcasts.html">More podcasts&hellip;</a></p>';
	$('#podcasts').html(html);  
}
...
function do_search() {
	...
	var search_type = $('#dig-type').val();
	
	var search_reqtags = '';
	var search_tags = '';
	var search_param_type = 'all';
	var digging_for = '<h3 id="diggingfor">You went digging for: All types of music</h3>';
	
	switch(search_type) {
		case 'videos':
			search_reqtags = 'instrumental';
		...
		case 'games':
			...
			break;
		case 'podcasting':
			search_tags = 'male_vocals,female_vocals';
			search_param_type = 'any';
...
function do_advanced_search() {
	...
	var form = $('#'+form_id);
	var search_val = $('#advanced-dig-query').val();
	var search_lic = $('#advanced-dig-lic').val();
	var search_results = $('#dig-limit').val();
	var search_since = $('#dig-since').val();
	var search_sort = $('#dig-sort').val();
	var search_order = $('#dig-ord').val();
	var search_tags = $('#dig-tags').val();
...
jQuery(document).ready(function() {
	...
	if(jQuery('#homepage').length > 0) {
		// entry_search_button.click(do_entry_search);
	}
